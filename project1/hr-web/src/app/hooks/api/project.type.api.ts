...
export const project_typeClient = {
   findAll: (params?: BaseApiQueryType) => {
      return HttpClient.get<BaseApiResponseType>(
         API_ROUTES.PROJECT_TYPE.INDEX,
         params
      )
   },
   getMore: () => {
      return HttpClient.get<ProjectTypeType>(
   ...
   create: (payload?: Record<string, any>) => {
      return HttpClient.post<ProjectTypeType>(
         API_ROUTES.PROJECT_TYPE.INDEX,
         payload
      )
   },
   update: (payload: { id: string } & Record<string, any>) => {
      return HttpClient.patch<ProjectTypeType>(
         `${API_ROUTES.PROJECT_TYPE.INDEX}/${payload?.id}`,
         payload
      )
   },
   delete: (id: string) => {
      return HttpClient.delete(`${API_ROUTES.PROJECT_TYPE.INDEX}/${id}`)
   }
}
